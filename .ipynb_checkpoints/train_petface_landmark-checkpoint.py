...
# ===== Config loader =====
def build_pid_mapping(split_csv):
    """从训练CSV收集所有pid，构建 稠密映射 {原始pid -> 0..C-1}"""
    import csv
    uniq, seen = [], set()
    with open(split_csv) as f:
        ...
        for row in r:
            if not row or len(row) < 2:
                continue
            pid = row[1]
            if isinstance(pid, str) and pid.lower() == "id":
                continue
            ...
            if pid not in seen:
                ...
                uniq.append(pid)
    uniq.sort()
    return {pid: i for i, pid in enumerate(uniq)}
...
def make_loader(img_root, split_csv, landmarks_json, img_size, batch_size, shuffle, pid_map=None):
    ds = PetFaceWithKpts(
        img_root=img_root,
        split_csv=split_csv,
    ...
    def collate(batch):
        xs, ys = [], []
        toT = AlignThenToTensor((img_size, img_size))
        for b in batch:
            x = toT(b["image"], b.get("landmarks"))
            y_raw = int(b["id"])  # 强制成 int
            if pid_map is not None:
                # 只保留在训练映射里的样本，避免越界
                if y_raw not in pid_map:
                    continue
                y = pid_map[y_raw]
            else:
                y = y_raw
    ...
    return DataLoader(ds, batch_size=batch_size, shuffle=shuffle, num_workers=2, collate_fn=collate)
...
def main():
    ...
    # ==== Data ====
    pid_map = build_pid_mapping(cfg["TRAIN_CSV"])
    for extra_csv in [cfg.get("VAL_QUERY_CSV"), cfg.get("VAL_GALLERY_CSV")]:
        if extra_csv:
            extra_map = build_pid_mapping(extra_csv)
            for pid in extra_map.keys():
                if pid not in pid_map:
                    pid_map[pid] = len(pid_map)
    num_classes = len(pid_map)

    train_loader = make_loader(
        cfg["IMG_ROOT"], cfg["TRAIN_CSV"], cfg.get("LANDMARKS_JSON"),
        cfg["IMG_SIZE"], cfg["BATCH_SIZE"], shuffle=True, pid_map=pid_map
    )

    # 验证集如果用 val_query.csv，其中会有训练没见过的ID，已在 collate 里自动丢弃这些样本；
    # 如果你想简单起见，也可以先把这里改成 cfg["TRAIN_CSV"] 做sanity-check。
    val_loader = make_loader(
        cfg["IMG_ROOT"], cfg.get("t_CSV", cfg["TRAIN_CSV"]),
        cfg.get("LANDMARKS_JSON"), cfg["IMG_SIZE"], cfg["BATCH_SIZE"],
        shuffle=False, pid_map=pid_map
    )
#     val_loader = make_loader(
#     cfg["IMG_ROOT"],
#     cfg["TRAIN_CSV"],                     # << 先用训练集做验证
#     cfg.get("LANDMARKS_JSON"),
#     cfg["IMG_SIZE"],
#     cfg["BATCH_SIZE"],
#     shuffle=False,
#     pid_map=pid_map
